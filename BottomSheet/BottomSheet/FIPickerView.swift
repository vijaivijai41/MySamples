...
//  FICommon
//
//  Created by Vijayakumar on 04/07/19.
...
open class FIPickerView: UIView,UIPopoverPresentationControllerDelegate {
    ...
    /// Load view from xib
    ///
    /// - Returns: return FIPickerView
    class func loadFromNib() -> FIPickerView? {
        let bundle = Bundle.init(for: FIPickerView.self)
        let view = bundle.loadNibNamed("FIPickerView", owner: self, options: nil)?.first as? FIPickerView
    ...
    @IBAction func cancelAction(_ sender: Any) {
        cancelHandler?()
        FIPickerView.actionSheetBackgroundTapped()
    }
    ...
    open class func showPicker(dataSource: ActionSheetsDataSource,selectedIndex:Int ,orgin:UIView?,doneHandler:DoneActionHandler?,cancelHandler:ButtonActionHandler?) ->  FIPickerView? {
        
        let alertController = UIAlertController(title: "", message: nil, preferredStyle: .actionSheet)
        let pickerView = FIPickerView.loadFromNib()
        ...
        UIApplication.getTopViewController()?.present(alertController, animated: true) {
            ...
            alertController.view.superview?.subviews.first?.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(FIPickerView.actionSheetBackgroundTapped)))
...
extension FIPickerView :UIPickerViewDelegate,UIPickerViewDataSource {
